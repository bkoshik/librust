...
pub fn open(path: &str, open_flags: OpenFlags, permission_flags: PermissionFlags) -> Result<i64> {
    return syscall(
        ...
        &create_args!(-2, path.as_ptr() as i64, open_flags.bits() as i64, permission_flags.bits() as i64)
    )
}

pub fn openat(dfd: i64, path: &str, open_flags: OpenFlags, permission_flags: PermissionFlags) -> Result<i64> {
    return syscall(
        SyscallNumber::Openat,
        &create_args!(dfd, path.as_ptr() as i64, open_flags.bits() as i64, permission_flags.bits() as i64)
    )
}
